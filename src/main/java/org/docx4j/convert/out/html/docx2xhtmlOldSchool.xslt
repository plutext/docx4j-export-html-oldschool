...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!-- Use method="html" if you need IE9 to display quotes properly (it
   		still might not use UTF 8 though?). Chrome works either way. 
   		NB, with IE9, and XML output, 
   		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
   		doesn't help.
   		(For HTML output that is inserted automatically)-->
...
<xsl:template match="/w:document">
	...
	<xsl:variable name="dummy"
		select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.log('/pkg:package')" />
		...
		<head>
			<style>
				<xsl:comment>
					...
					p.tab0 {text-indent: 0cm;}
					p.tab1 {text-indent: 1cm;}
					p.tab2 {text-indent: 2cm;}
...
<xsl:template name="pretty-print-block">
  ...
  <xsl:template match="w:p">
  	...
  	<xsl:call-template name="pretty-print-block"/>
		...
		<xsl:variable name="pStyleVal" select="string( w:pPr/w:pStyle/@w:val )" />  <!-- could do without this arg, give $pPrNode -->
		...
		<!-- does this look like a paragraph, or a heading? -->  
		<xsl:variable name="textContents">
			<xsl:value-of select="string(.)" />  
		</xsl:variable>

			<xsl:copy-of 
				select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.message( string(.) )" />  	  		

	  	<xsl:copy-of select="
	  		java:org.docx4j.convert.out.html.HtmlExporterOldSchool.createBlockForPPr( 
 							$wmlPackage, $pPrNode, $pStyleVal, $childResults, string-length($textContents),
 							$numId, $levelId )" />
  	...
  	<xsl:choose>
  		<xsl:when test="w:rPr">
			...
			<xsl:variable name="rPrNode" select="w:rPr" />  	
		  	...
		  	<xsl:copy-of select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.createBlockForRPr( 
  ...
  <xsl:template match="wp:inline|wp:anchor">
  	...
  	<xsl:copy-of select="java:org.docx4j.model.images.WordXmlPictureE20.createHtmlImgE20( 
  		...
  		<xsl:otherwise>
			...
			<xsl:copy-of 
				select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.notImplemented(., ' without pic:pic ' )" />  	  		
  		</xsl:otherwise>  	
  	</xsl:choose>
	...
	<xsl:template match="w:pict">
		...
		<xsl:choose>
			...
			<xsl:copy-of 
				select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.notImplemented(., ' without v:imagedata ' )" />  	  		
			</xsl:otherwise>
		</xsl:choose>  			
...
<xsl:template match="w:bookmarkEnd" />
  ...
  <xsl:template match="w:footnoteReference">  
    <xsl:variable name="fn"><xsl:value-of select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.getNextFootnoteNumber($modelStates)"/></xsl:variable>
  ...
  <xsl:template match="w:endnoteReference ">  
    <xsl:variable name="fn"><xsl:value-of select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.getNextEndnoteNumber($modelStates)"/></xsl:variable>
  	<span style="vertical-align: top; font-size: xx-small">
...
<!--  tmp bookmarks -->
  ...
  <xsl:template match="w:fldSimple" >
		<xsl:copy-of 
			select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.notImplemented(., 'no support for fields' )" />  	
  </xsl:template>
  <xsl:template match="w:fldChar" >
		<xsl:copy-of 
			select="java:org.docx4j.convert.out.html.HtmlExporterOldSchool.notImplemented(., '' )" />  	
  </xsl:template>
